# Prediction Stage C DFlow Guarded Live Runbook

## Purpose

Define Stage C guarded-live operating procedures for `PM-C-001`, including conservative risk caps, approval control, dry-run evidence, and no-naked-exposure enforcement.

## Scope

- DFlow adapter guarded mode only (`dry_run` and `guarded_live`).
...
- swap submission metadata (`idempotencyKey`, deduplication flag)

Persist artifact snapshots into your evidence bundle for Stage C review.
...
4. Archive artifact JSON and attach to PM-C-001 evidence.
...
Use for `prediction_dflow_naked_exposure_violation`.
...
- Guarded Stage C evidence generation:
  - `bash infra/ci/validate-prediction-stage-c-dflow-guarded.sh`
